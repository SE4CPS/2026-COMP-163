...
def get_flowers():
    ...
    cur = conn.cursor()
    cur.execute("SELECT id, name, last_watered, water_level, min_water_required FROM team7_flowers ORDER BY id;")
...
def get_flowers_needing_water():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, name, last_watered, water_level, min_water_required FROM team7_flowers WHERE water_level < min_water_required ORDER BY id;")
    flowers = cur.fetchall()
    cur.close()
    conn.close()
...
def add_flower():
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO team7_flowers (name, last_watered, water_level, min_water_required) VALUES (%s, %s, %s, %s)",
...
def update_flower(id):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE team7_flowers SET last_watered = %s, water_level = %s WHERE id = %s",
